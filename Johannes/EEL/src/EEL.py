...
def init():
    ...
    import os
    t_additional_files = f"{os.path.split(__file__)[0]}\\{additionalFilesPath}"
...
def get_dir_path() -> str:
    ...
    root.withdraw()
    root.wm_attributes('-topmost', 1)
    t_initialdir_path = f"{path.split(file)[0]}"
...
def load_gui_elements() -> list[dict[str, Any]]:
    ...
    for o in g_intermediary.getObjects():
        t_objekts.append(convert_attribut_to_js_data(
...
def create_btn() -> dict[str, Any]:
    ...
    t_data = convert_attribut_to_js_data(
        g_intermediary.getObject(t_id).getAttributesAsDictionary())
    # print(t_data)
    return t_data
