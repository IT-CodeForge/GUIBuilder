...
class ETK_system_gui_generator:
    ...
    def generate_file(self, etk_objects: tuple[IBaseObject]) -> str:
        ...
        my_event_list: list[tuple[IBaseObject, str, str]] = self.__generate_event_list(etk_objects)
        ...
        for ast_object in my_class_body: # type:ignore
            if type(ast_object) == FunctionDef: # type:ignore
                ...
                if ast_object.name == "_add_elements":
                    ast_object.body = self.__generate_attribute_creation(etk_objects) + self.__generate_event_binds(my_event_list)
        ...
        for etk_object in etk_objects:
            ...
            for attribute in attributes:
                ...
                if attribute.startswith("event_"):
                    ...
                    retval.append((etk_object, my_etk_event, intermediary_event))
    ...
    def __generate_event_funcs(self, event_list: list[tuple[IBaseObject, str, str]]) -> list[stmt]:
        retval: list[stmt] = []
    ...
    def __generate_attribute_creation(self, etk_objects: tuple[IBaseObject]) -> list[stmt]:
        ...
        for etk_object in etk_objects:
            ...
            if type(etk_object) == ITimer:
                retval.append(ast_gen.timer(etk_object, "timer"))
                continue
