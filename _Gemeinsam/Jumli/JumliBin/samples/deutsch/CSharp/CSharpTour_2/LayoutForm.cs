...
namespace CSharpXXRIS
{
	...
	public class LayoutForm : Form
	{
		...
		// erstellen einer Referenz zu unserem GameGrid
		...
		{
			//
			// Dies wird für den Support des Windows Form Designers benötigt
			//
			InitializeComponent();

			//
			// Egal welcher Constructor Code, immer erst die "InitializeComponent" aufrufen
			//
			SetUpGame();
		}

		/// <summary>
		/// Aufräumen von allen Resourcen die je benutzt wurden
		/// </summary>
		...
		/// <summary>
		/// Der folgende Code wird benötigt wenn man das Projekt
		///	mit einem Designer verändern möchte - 
		/// der Code sollte nicht mit dem Codeeditor verändert werden
		/// </summary>
		...
		#endregion
		/// <summary>
		/// Hier hört der generierte Code auf
		/// </summary>
		...
		/// <summary>
		/// Hier die Main-Methode als zentraler Einstiegspunkt
		/// </summary>
		...
		private void DrawScreen()
		{
			...
			// erst werden die festen Steine gezeichnet
			...
			// jetzt das zeichnen des sich bewegenden Shapes
		...
		private void LayoutForm_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			...
			else
			{
				switch(strKeyPress.ToUpper())
				{
					...
					case "ENTER":
						// erzeugen eines Shapes um damit zu starten
						SetUpGame();
		...
		private void GameTimer_Tick(object sender, System.EventArgs e)
		{
			...
			else
			{
				int intXCoordinate;
				int intYCoordinate;
				// das aktuelle Shape bewegt sich nicht mehr,
				// ist es noch im Spielfeld? Wenn nicht ist das Spiel zuende!!!!!!!!!
				for (int i=0;i<4;i++)
				{
				...
				if (!bolIsGameOver)
				{
					...
					// zeichen der entgültigen Position des Shapes
					for (int i=0;i<4;i++)
					{
						intXCoordinate = rctShape[i].X;
						intYCoordinate = rctShape[i].Y;
						intYCoordinates[i] = intYCoordinate/10;
						// kopieren des Shapes in das GamegridArray
						grdGameGrid.SetShapeLocation(intYCoordinate/10,intXCoordinate/10,rctShape[i],intShapeType);
					}
					// jetzt wird das Array anhand der Y-Koordinaten sortiert, angefangen mit den kleinen
					// zu den großen
					Array.Sort(intYCoordinates);
					for (int i=0;i<4;i++)
					{
						bolIsRowFull = true;
						// jetzt schauenwir ob der Spielstein eine neue Reihe füllt
						// falls das der Fall ist löschen wir die Reihe und rutschen mit
						// allen restlichen Spielsteinen eine Reieh runter
						...
						if (bolIsRowFull)
						{
							// runter schieben der Reihe							
							for (int k=intYCoordinates[i];k>0;k--)
							{
								// jetzt werden alle Koordinaten der Steine upgedated								
								for (int l=0;l<intNumberOfCols;l++)
								{
									// setzen des Wertes in die Reihe darunter
									grdGameGrid.DropRowsDown(k,l);
								}
							}
							
							grdGameGrid.SetTopRow();
							// update des Zählerstandes
					...
					shpShape.bolShapeMoving = true;
					DrawNextShape();
					// zurücksetzen der Spielgeschwindigkeit
					GameTimer.Interval = intGameSpeed;
					bolIsDropped = false;
				}
				else
		...
		{
			// initialisieren der variablen Werte
			intGameSpeed = 100;
			intBonusHeight = animationCanvas.Height-1;
			intDropRate = 5;
			intBonusStep = 5;
