...
class BSprite:
    ...
    def group_as_sprite(self, canvas_item_list:list[BCanvasItem], sprite_anchor:vector2d=vector2d(0,0)):
        self.sprite_list = canvas_item_list

    def load_sprite(self, file_path_with_file_name:str, canvas:TGCanvas):
        self.delete_sprite_data()
        with open(file_path_with_file_name, "r") as openfile:
            json_object = json.load(openfile)
            for canvas_item_data in json_object:
                canvas_item = BCanvasItem(canvas, "json" + canvas_item_data.get("item_type"), canvas_item_data)
